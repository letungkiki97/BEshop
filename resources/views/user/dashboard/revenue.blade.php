            ...
            <div class="col-xs-12">
                <table class="table table-striped table-bordered">
                    <thead>
                        <tr>
                            <th>{{ __('table.id') }}</th>
                            <th>{{ __('common.sales_agent') }}</th>
                            <th>{{ __('sales_order.date') }}</th>
                            <th>{{ __('sales_order.status') }}</th>
                            <th>{{ __('common.customer') }}</th>
                            <th>{{ __('common.product') }}</th>
                            <th>{{ __('sales_order.amount') }}</th>
                            <th>{{ __('delivery.delivered') }}</th>
                        </tr>
                    </thead>
                    <tbody>
                        @if(count($order))
                        ...
                        @foreach($order as $o)
                        <tr>
                            <td><a href="{{ url('sales_order', $o->id) }}/edit" target="_blank">{{$o->tracking_number}}</a></td>
                            <td>{{$o->salesPerson->full_name}}</td>
                            <td>{{$o->date}}</td>
                            <td>{{$o->status}}</td>
                            ...
                            <td>
                                ...
                                <p>{{ $p->item->product_sku . ' - ' . $p->item->product_name . ' - ' . $p->quantity . 'pc' }}</p>
                                @endforeach
                                @endif
                            </td>
                            <td>{{number_format($o->after_discount)}}</td>
                            <td>{{number_format($o->delivered)}}</td>
                        </tr>
                        @endforeach
                        @else
                        <tr>
